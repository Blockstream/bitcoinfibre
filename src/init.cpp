...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-udpmulticasttx=<if>,<ip/host>:<port>,<bw>,<txn_per_sec>[,<ttl>,<depth>,<dscp>,<interleave>]", "Transmit multicast-addressed mesages to <ip/host>:<port> through interface <if> with bandwidth <bw> in bps and TTL <ttl> (3 by default). Iteratively transmit the past <depth> blocks. If <depth> is set to 0, iterate over the full chain. If <txn_per_sec> is > 0, send this number of txns per second in addition to blocks. Optionally define a DSCP for marking IP packets. Interleave FEC chunks of blocks covering a window of <interleave> seconds. If <interleave> is set to 0, do not interleave FEC chunks.", ArgsManager::ALLOW_ANY, OptionsCategory::UDP);
