...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-blocksonly", strprintf("Whether to reject transactions from network peers. Automatic broadcast and rebroadcast of any transactions from inbound peers is disabled, unless '-whitelistforcerelay' is '1', in which case whitelisted peers' transactions will be relayed. RPC transactions and transactions received from udpmulticast peers are not affected. (default: %u)", DEFAULT_BLOCKSONLY), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-udpfecstatstimeout=<timeout>", "Change timeout for printing FEC chunk stats of a partial block when using debug=fec. Set <timeout> in seconds.", ArgsManager::ALLOW_ANY, OptionsCategory::UDP);
#ifdef USE_UPNP
#if USE_UPNP
