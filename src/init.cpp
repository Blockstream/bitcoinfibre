...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-udpmulticasttx=<if>,<ip/host>:<port>,<bw>,<send_txns>[,<ttl>,<depth>,<dscp>]", "Transmit multicast-addressed mesages to <ip/host>:<port> through interface <if> with bandwidth <bw> and TTL <ttl> (3 by default). Iteratively transmit the past <depth> blocks. If <depth> is set to 0, iterate over the full chain. If <send_txns> is 1, send txns in addition to blocks. Optionally define a DSCP for marking IP packets.", ArgsManager::ALLOW_ANY, OptionsCategory::UDP);
